...
import 'package:flatypus/state/controllers/task_for_day_controller.dart';
...
class AssignedTasks extends ConsumerWidget {
  const AssignedTasks(
      {super.key, required this.title, this.userId, this.taskForDay = false});
  final String title;
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          return const Center(
            child: Text('Failed to fetch the Tasks'),
          );
        }
      ...
      child: Row(
        ...
        children: [
          ...
          if (!taskForDay)
            SizedBox(
              child: Consumer(
                builder: (context, ref, child) => CustomOutlineNavigationButton(
                  label: 'All Tasks',
                  foregroundColor: AppColors.white.withAlpha(200),
                  function: () {
                    ref.read(selectedPageProvider.notifier).state = AppScreens.tasks.index;
                  },
                ),
              ),
            )
