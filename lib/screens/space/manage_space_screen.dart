...
import 'package:flatypus/common/widgets/custom_icon_button.dart';
...
import 'package:flatypus/screens/space/methods/space_methods.dart';
...
class ManageSpaceScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final tasksForSpace = allTasks
        .where((tsk) => tsk.spaceId == space.id && tsk.scheduledDate != null)
        .toList();
          ...
          Row(
            ...
            children: [
              ...
              CustomTextNIconButton(
                label: 'Add Task',
                icon: Icons.add_task,
                onTap: () => SpaceMethods.onAddTaskForSpaceButtonTap(
                    ref: ref, space: space),
                foregroundColor: AppColors.yellowAccent2,
              ),
            ],
  ...
  _currentSchedule({required List<TaskModel> tasks}) {
    return [
      ...
      if(tasks.isNotEmpty)
      ...tasks.map((tsk) => TaskScheduleCard(task: tsk)),
      if(tasks.isEmpty)
        const Padding(
