...
def send_group_notification(event: firestore_fn.Event[firestore_fn.DocumentSnapshot | None]) -> None:
    ...
    try:
        ...
        db = firestore.client()
        house_ref = db.collection(Collections.HOUSES).document(houseId)
        ...
        sender_ref = db.collection(Collections.USERS).document(sender_id)
        ...
        for member_id in members:
            if member_id != sender_id:  # Exclude sender
                user_ref = db.collection(Collections.USERS).document(member_id)
